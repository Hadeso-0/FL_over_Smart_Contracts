...
def setup_simulation(s):
    np.random.seed(s)
    training_data,testing_data,edge_data = get_data(s, n_edges)

    edge_nets = init_networks(n_inputs_network, n_outputs_network, hidden_layers_sizes, n_edges)
...
def run_stage(testing_data, edge_nets, edge_data,
    ...
    results_temp = run_local(testing_data, edge_nets, edge_data, n_local_updates, s)
...
def run_federation(edge_nets, edge_data):
    ...
    agg_weights, agg_biases = federate_edges([[a.weights for a in edge_nets], [a.biases for a in edge_nets]], edge_cardinalities)
...
def read_network(net_dict):
    ...
    hidden_layers_sizes = [net_dict["sizes"][i] for i in range(1, net_dict["num_layers"]-1)]
    ret = nn.Network([n_inputs_network, *hidden_layers_sizes, n_outputs_network])
