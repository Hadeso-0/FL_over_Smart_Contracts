...
def call_init():
    ...
    db.params.replace_one({'s': s}, {'s': s, 'n_global_updates': n_global_updates, 'n_local_updates': n_local_updates}, upsert=True)
    db.networks.replace_one({'s': s}, {'s': s, 'edge_nets': [controler.write_network(e) for e in edge_nets], 'edge_data': [dp.toList(edge_data)]}, upsert=True)
    db.data.replace_one({'s': s}, {'s': s, 'testing_data': dp.toList(testing_data)}, upsert=True)
...
def runStageAPI():
    ...
    return{'weights': [dp.toInt(w) for w in net_weights], 'biases': [dp.toInt(b) for b in net_biases]}
...
def submitParamsAPI():
    ...
    for e in range(len(edge_nets)):
        edge_nets[e]["weights"] = [dp.toFloat(w) for w in res_weights]
        edge_nets[e]["biases"] = [dp.toFloat(w) for w in res_biases]
        db.networks.replace_one({'s': s}, {'s': s, 'edge_nets': [e for e in edge_nets], 'edge_data': edge_data}, upsert=True)
